...
import { CalendarClock, CheckCircle2, XCircle, Clock, MapPin, ArrowLeft, Loader2, Send, Navigation, AlertTriangle } from 'lucide-react';
...
export default function AppointmentScheduler({ applicationId, userId, myRole, onBack }: SchedulerProps) {
  ...
  const handleAcceptTime = async () => {
    ...
    if (otherAgreed) finalStatus = 'confirmed';
  ...
  const handleShareLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      const updateData = myRole === 'student' 
        ? { student_lat: pos.coords.latitude, student_lng: pos.coords.longitude } 
        : { tutor_lat: pos.coords.latitude, tutor_lng: pos.coords.longitude };

      await supabase.from('appointments').update(updateData).eq('id', appt.id).select();
      setGpsLoading(false);
    }, () => {
  ...
  const handleCancel = async () => {
    ...
    const { data } = await supabase.from('appointments').update({ 
      status: 'cancelled', student_agreed: false, tutor_agreed: false 
      ...
      <div className="p-4 flex-1 overflow-y-auto">
        ...
        {appt.status === 'cancelled' && (
          <div className="bg-red-500/10 border border-red-500/30 p-4 rounded-xl text-center">
            ...
            <p className="text-[10px] text-red-300 mt-2 mb-4">The previous proposal was cancelled.</p>
            <div className="flex flex-col gap-2">
               <input type="datetime-local" className="w-full bg-black/50 border border-red-500/30 rounded p-2 text-xs text-white" onChange={(e) => setDateInput(e.target.value)}/>
              <button onClick={handlePropose} disabled={!dateInput} className="w-full bg-red-500/20 hover:bg-red-500/30 text-red-200 text-xs py-2 rounded font-bold">START NEW PROPOSAL</button>
            </div>
          </div>
        )}
        ...
        {appt.status === 'confirmed' && (
          <div className="space-y-4 animate-in zoom-in">
            ...
            <div className="bg-white/5 p-3 rounded-xl border border-white/10">
              ...
              {myLat ? (
                 <div className="flex items-center gap-2 text-emerald-400 text-xs font-bold bg-emerald-900/20 p-2 rounded"><CheckCircle2 size={14}/> Location Sent Successfully</div>
              ) : (
                 <button onClick={handleShareLocation} disabled={gpsLoading} className="w-full bg-blue-600 hover:bg-blue-500 text-white py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-2 shadow-lg">
                   {gpsLoading ? <Loader2 className="animate-spin" size={14}/> : <Navigation size={14}/>} SHARE EXACT GPS
                 </button>
              )}
            </div>
            ...
            <div className="bg-white/5 p-3 rounded-xl border border-white/10">
              ...
              {theirLat ? (
                <div>
                   ...
                   <a href={`https://maps.google.com/maps?q=${theirLat},${theirLng}`} target="_blank" rel="noreferrer" className="block text-center bg-emerald-600 hover:bg-emerald-500 text-black font-bold py-2 rounded text-xs transition-colors">OPEN IN GOOGLE MAPS</a>
                </div>
              ) : (
                <div className="text-xs text-yellow-500/80 flex items-center gap-2 bg-yellow-500/10 p-2 rounded"><Loader2 size={12} className="animate-spin"/> Waiting for partner...</div>
              )}
            </div>
            <button onClick={handleCancel} className="w-full text-center text-[10px] text-red-400 hover:text-red-300 mt-4 underline decoration-red-900">Cancel Appointment</button>
          </div>
        )}
        ...
        {appt.status === 'negotiating' && (
          <div className="bg-white/5 border border-white/10 p-4 rounded-xl">
              ...
              <div className="mb-6 bg-black/40 rounded-lg p-3 border border-white/5">
                <div className="flex justify-between items-center mb-2">
                   <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider">
                     {isMyProposal ? "You Proposed:" : "Partner Proposed:"}
                   </p>
                   {!isMyProposal && <div className="bg-yellow-500/20 text-yellow-400 text-[10px] px-2 py-0.5 rounded font-bold animate-pulse">ACTION REQUIRED</div>}
                </div>
                
                <div className="text-sm font-bold text-white flex items-center gap-2 mb-3 bg-zinc-800 p-2 rounded border border-white/10">
                  <Clock size={16} className="text-emerald-400"/> 
                  {new Date(appt.proposed_date).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })}
                </div>
                
                {!isMyProposal ? (
                  <button onClick={handleAcceptTime} disabled={loading} className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-2.5 rounded-lg text-xs flex items-center justify-center gap-2 shadow-lg hover:shadow-emerald-900/20 transition-all">
                    {loading ? <Loader2 size={12} className="animate-spin"/> : <CheckCircle2 size={14} />} ACCEPT THIS TIME
                  </button>
                ) : (
                  <div className="text-[10px] text-yellow-500 flex items-center justify-center gap-1.5 bg-yellow-900/10 p-1.5 rounded">
                     <Loader2 size={10} className="animate-spin"/> Waiting for partner response...
                  </div>
                )}
              </div>
            ) : (
            ...
            <div className="pt-4 border-t border-white/10">
              ...
              <input type="datetime-local" className="w-full bg-black/50 border border-white/20 rounded-lg p-2.5 text-xs text-white mb-3 focus:border-emerald-500 outline-none" onChange={(e) => setDateInput(e.target.value)}/>
              <button onClick={handlePropose} disabled={!dateInput || loading} className="w-full bg-white/10 hover:bg-white/20 text-white font-bold py-2.5 rounded-lg text-xs border border-white/20 flex items-center justify-center gap-2 transition-all disabled:opacity-50">
                {loading ? <Loader2 className="animate-spin" size={12}/> : <Send size={12}/>} {appt.proposed_date ? 'SEND COUNTER-OFFER' : 'SEND PROPOSAL'}
              </button>
            </div>
          </div>
        )}
